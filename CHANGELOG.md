...
### Added
- **2FA-Code-Autofill in der Extension** — beim Fokussieren eines One-Time-Code-Feldes (`autocomplete="one-time-code"` oder Heuristik) bietet die Fill-Box den aktuellen TOTP-Code des passenden Eintrags an (bevorzugt der zuletzt in diesem Tab ausgefüllte Login), mit Live-Countdown und automatischem Nachladen des nächsten Codes. Nach einem normalen Login-Fill eines Eintrags mit TOTP-Secret wird der Code automatisch in die Zwischenablage kopiert („2FA code copied"). Neue Einträge im Popup akzeptieren ein 2FA-Secret als Base32 oder `otpauth://`-URI (`parseOtpauthUri` in shared-ts). Das Secret verlässt nie den Background-Worker
- **Kreditkarten- & Identitäten-Autofill in der Extension** — der Feld-Classifier erkennt Checkout-/Adressfelder (autocomplete-Tokens + EN/DE-Heuristiken); der Picker zeigt nur Namen + unkritischen Hint (`•••• 1234`), erst nach Auswahl wird entschlüsselt und alle erkannten Felder des Formulars gefüllt (inkl. Selects für Ablaufmonat/Land). Popup kann typisierte Einträge anlegen (Login/Karte/Identität/Notiz), Detailansicht rendert alle Feldtypen mit Maskierung
- **Wallet-Tab in der App** — eigener Bereich in der Bottom-Navigation, der Karten, Identitäten, Bankkonten und Lizenzen gruppiert verwaltet, mit Schnellanlage pro Typ
- **„Sign in with …"-Merker, geräteübergreifend** — Einträge haben ein neues E2E-verschlüsseltes Feld `sign_in_with` (Google/Apple/Microsoft/GitHub/Facebook), editierbar im Extension-Popup, in der App (Dropdown beim Login-Typ) und sichtbar in `pwmgr get`. Die Extension erkennt SSO-Logins weiterhin automatisch (Klick-Heuristik + Beobachtung der OAuth-Navigation, inkl. `window.open`-Popups) und schreibt den Provider in den passenden Eintrag — damit synct der Merker über alle Geräte; ohne passenden Eintrag bleibt ein gerätelokaler Fallback. Beim nächsten Besuch erscheint der Badge „You sign in here with Google →", der den Provider-Button der Seite anklickt; × vergisst lokal **und** im Eintrag
- **Papierkorb (Soft-Delete)** — Löschen verschiebt Einträge in den Papierkorb statt sie unwiderruflich zu entfernen; Wiederherstellen und endgültiges Löschen in App (Settings → Trash) und CLI (`pwmgr trash list|restore|purge`), automatische Bereinigung nach 30 Tagen (Server-Purge-Loop)
- **Favoriten** — Einträge lassen sich markieren (Stern in Extension-Popup, App-Detail und CLI `pwmgr favorite`) und sortieren überall zuerst
- **Eintrags-Historie** — jede Änderung sichert den vorherigen verschlüsselten Stand (bis zu 20 Versionen, inkl. der damaligen per-User-Keys → E2E bleibt intakt, da jedes Update einen frischen Data-Key wrappt). Anzeigen/Entschlüsseln/Wiederherstellen im History-Sheet der App und via `pwmgr history <name> [--restore <id>]`; Restore ist rein server-seitig und versioniert vorher den aktuellen Stand
- **Passwort-Health-Report** — vault-weite Analyse auf schwache, wiederverwendete, alte und geleakte Passwörter, mit Gesamt-Score. In der Extension als eigener `./health`-Tab (Report entsteht im Background — Passwörter erreichen das Popup nie) und in der App unter Settings → Password health. Der Breach-Check ist opt-in und nutzt die HIBP-Range-API mit k-Anonymität: nur die ersten 5 Zeichen des SHA-1-Hashes verlassen das Gerät. Die Stärke-Heuristik ist ein 1:1-Port von `CheckStrength` (CLI) nach TS und Dart mit identischen Test-Vektoren
- Neue Endpoints: `GET /entries/trash`, `POST /entries/{id}/restore`, `DELETE /entries/{id}/permanent`, `PUT /entries/{id}/favorite`, `GET /entries/{id}/versions[/{vid}]`, `POST /entries/{id}/versions/{vid}/restore`; Migration `000007` (deleted_at, favorite, entry_versions, entry_version_keys)

- **Tiefere Suche in der App** — der Suchindex der Eintragsliste indexiert jetzt neben dem Benutzernamen auch E-Mail, Namen, Firma, Telefon, Adresse, Notizen, Custom-Field-Labels/-Werte, Karteninhaber sowie die letzten 4 Stellen von Karte/IBAN/Kontonummer (Passwörter, CVV und volle Nummern bewusst nicht). AutoFill-Sync lässt Einträge mit Passwort ohne Benutzernamen nicht mehr weg und liefert den Eintragstyp mit

### Fixed
- **Speichern-Erkennung auf Flutter-Web-/SPA-Logins** — der Save-Detector hörte nur auf native `submit`-Events, die SPAs nie feuern. Er snapshottet Zugangsdaten jetzt aus composed Input-Events (sieht auch Shadow-DOM-Felder wie in Flutter Web) und bietet das Speichern bei Enter, Mousedown außerhalb eines Textfeldes (vor dem Feld-Clearing der SPA) oder Seitenwechsel an — die Save-Bar erscheint damit beim **ersten** Klick auf den Login-Button
- **Fill-Overlay verdeckte den Login-Button** — das Vorschlags-iframe prüft seine Zielfläche jetzt per `elementsFromPoint` und klappt über das Feld, wenn darunter interaktive Elemente liegen; es folgt dem Feld bei Scroll/Resize, Klicks auf die Freifläche schließen es, und Outside-Dismiss läuft auf capture-`mousedown`, bevor die Seite den Klick verarbeitet
- **Passwort-Update via Save-Bar warf andere Eintragsfelder weg** — „Update existing" überschrieb den Datenblob mit nur Benutzername+Passwort; jetzt wird in die entschlüsselten Bestandsdaten gemergt, sodass TOTP-Secret, Notizen und `sign_in_with` erhalten bleiben
- **App-Edit verlor unbekannte Datenfelder** — der Bearbeiten-Screen der App trägt jetzt alle Felder, die das Formular nicht verwaltet (z.B. TOTP-Metadaten anderer Clients), beim Speichern unverändert weiter
