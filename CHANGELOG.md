...
### Added
- **Share-Links in der CLI/TUI** (vorher nur Flutter): im Vault-Screen einen Eintrag markieren und mit `L` einen Zero-Knowledge-Link erzeugen — Ablauf wählbar (1 Tag / 7 / 30 / 90 Tage / 1 Jahr / Never). Das Ergebnis wird als **ASCII-QR-Code direkt im Terminal** plus voller URL angezeigt (und in die Zwischenablage kopiert). Der Link-Schlüssel lebt nur im URL-Fragment und erreicht den Server nie; identisches Format wie die Flutter-App, also vom Web-Viewer lesbar
