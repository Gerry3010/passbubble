...
func (s *Server) buildRouter() http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/entries/trash", h.ListTrash)
			...
			r.Delete("/entries/{id}/permanent", h.PurgeEntry)
			r.Put("/entries/{id}/favorite", h.SetFavorite)
			r.Get("/entries/{id}/versions", h.ListVersions)
			r.Get("/entries/{id}/versions/{vid}", h.GetVersion)
			r.Post("/entries/{id}/versions/{vid}/restore", h.RestoreVersion)
